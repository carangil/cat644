...
unsigned char comm_ioctl(devid dev, unsigned char num, void* vparms)
{
	comm_ioctl_parms_t* parms = (comm_ioctl_parms_t*) vparms;
	
	if (num == COMM_IOCTL_INITBAUD) {
	
		if (parms->enable_115200)
			comm_init_115200();
		else
			comm_init( parms->baud);

		return 1;
	}
		
	else return 0;  //unknown ioctl
}


#define COMM_SEND_READY  ( UCSR0A& _BV(UDRE0) )

void comm_putc(devid dev, unsigned char c)
{
	//wait for it
...
unsigned char comm_kbhit(devid dev)
{
